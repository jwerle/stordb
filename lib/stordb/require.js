...
let callsite = nativeRequire('util/callsite').exports
...
function require (module) {
  let ext = null;
  if ('require' == module) { return require; }
  if ('.' == module[0]) {
    module = resolve(__DIR__, module);
  }
  ext = extname(module);
  if (extensions[ext]) {
...
function resolve (root, mod) {
  var rbase = basename(root);
  var mbase = basename(dirname(mod));
  if (rbase == mbase) {
    return path.resolve(root, basename(mod));
  }
  return mod;
}
